...
class McWss {
    ...
    this.wss.on('connection', (ws: WebSocket, req: IncomingMessage) => {
      ...
      ws.on('message', async buffer => {
        ...
        sendMsg = this.ctx.i18n.render(
          [this.conf.locale ? this.conf.locale : 'zh-CN'],
  ...
  private setupMessageHandler() {
    ...
    this.ctx.on('message', async session => {
      ...
      if (
        this.conf.sendToChannel.includes(
...
export function extractAndRemoveColor(input: string): {
  output: string;
  color: string;
} {
  const regex = /&(\w+)&/;
